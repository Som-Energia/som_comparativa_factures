...
def test_versions_endpoint_lists_statuses(template_store):
    add_valid_version(template_store, 2)
    add_valid_version(template_store, 3)
    client = create_app().test_client()
...
def test_create_version_clones_from_existing_version(template_store):
    ...
    response = client.post(
        ...
        json={"source_version": "v1", "target_version": "v2"},
    )
    ...
    assert "template_version: 2" in payload["files"]["content"]
...
def test_update_version_saves_a_draft(template_store):
    add_valid_version(template_store, 2)
    client = create_app().test_client()
    detail = client.get("/api/templates/comparison/versions/v2").get_json()
    ...
    response = client.put(
        "/api/templates/comparison/versions/v2",
        json={"files": detail["files"]},
    )
...
def test_preview_version_renders_html_from_unsaved_yaml(template_store):
    ...
    response = client.post(
        "/api/templates/comparison/versions/v2/preview",
        json={"files": detail["files"]},
    )
